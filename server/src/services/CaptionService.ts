...
export type CaptionSource = "native" | "whisper" | "piped";
...
/** Path 1c-b — Piped/Invidious fallback (no yt-dlp, no cookies, no IP block). Parallel, 5s budget. */
async function fetchPipedCaptions(videoId: string, lang = "en"): Promise<CaptionsResult | null> {
  const tryOnePiped = async (base: string): Promise<CaptionsResult | null> => {
    try {
      const streamsRes = await fetchWithTimeout(`${base}/streams/${videoId}`, 5000);
      if (!streamsRes.ok) return null;
      const streams = (await streamsRes.json()) as {
        subtitles?: Array<{ url?: string; mimeType?: string; name?: string; code?: string; autoGenerated?: boolean }>;
        title?: string;
      };
      const caps = streams.subtitles ?? [];
      if (!Array.isArray(caps) || caps.length === 0) return null;
      const norm = (s?: string) => (s ?? "").toLowerCase().split("-")[0];
      const preferred = caps.find((c) => norm(c.code ?? c.name) === lang && !c.autoGenerated);
      const autoPref = caps.find((c) => norm(c.code ?? c.name) === lang);
      const picked = preferred ?? autoPref ?? caps.find((c) => !c.autoGenerated) ?? caps[0];
      if (!picked?.url) return null;
      const r = await fetchWithTimeout(picked.url, 5000);
      if (!r.ok) return null;
      const text = await r.text();
      let segs: CaptionSegment[] = [];
      if (text.trim().startsWith("{")) {
        try {
          segs = parseJson3(JSON.parse(text));
        } catch {
          segs = parseVtt(text);
        }
      } else if (text.includes("<transcript") || text.includes("<text") || text.includes("<p")) {
        segs = parseTimedtextXml(text);
      } else {
        segs = parseVtt(text);
      }
      segs = sanitizeSegments(segs);
      if (segs.length === 0) return null;
      logger.info({ videoId, count: segs.length, via: `piped:${base}`, lang: picked.code ?? picked.name }, "piped captions hit");
      return {
        videoId,
        providerId: "youtube",
        source: "piped",
        language: picked.code ?? picked.name ?? lang,
        isAuto: !!picked.autoGenerated,
        captions: segs,
        srt: toSrt(segs),
        vtt: toVtt(segs),
      };
  ...
  // Only 2 most reliable, in parallel, 5s each — total 5s not 40s
  const pipedBases = ["https://pipedapi.kavin.rocks", "https://pipedapi.adminforge.de"];
  const results = await Promise.all(pipedBases.map((b) => tryOnePiped(b)));
  const hit = results.find((r) => r !== null);
  if (hit) return hit;
  // Invidious quick try (single, 5s) — not 4 sequential
  try {
    const base = "https://invidious.nerdvpn.de";
    const r = await fetchWithTimeout(`${base}/api/v1/captions/${videoId}`, 5000);
    if (r.ok) {
      const data = (await r.json()) as { captions?: Array<{ languageCode?: string; url?: string; isAutoGenerated?: boolean }> };
      const caps = data.captions ?? [];
      const norm = (s?: string) => (s ?? "").toLowerCase().split("-")[0];
      const picked = caps.find((c) => norm(c.languageCode) === lang && !c.isAutoGenerated) ?? caps.find((c) => norm(c.languageCode) === lang) ?? caps[0];
      if (picked?.url) {
        const capUrl = picked.url.startsWith("http") ? picked.url : `${base}${picked.url}`;
        const r2 = await fetchWithTimeout(capUrl, 5000);
        if (r2.ok) {
          ...
          } else if (text.includes("<transcript") || text.includes("<text")) {
            segs = parseTimedtextXml(text);
          } else {
            segs = parseVtt(text);
          }
          segs = sanitizeSegments(segs);
          if (segs.length > 0) {
            logger.info({ videoId, count: segs.length, via: `invidious:${base}` }, "invidious captions hit");
            return {
              videoId,
              providerId: "youtube",
              source: "native",
              language: picked.languageCode ?? lang,
              isAuto: !!picked.isAutoGenerated,
              captions: segs,
              srt: toSrt(segs),
              vtt: toVtt(segs),
            };
...
async function fetchNativeViaYtDlp(url: string, videoId?: string): Promise<CaptionsResult | null> {
  ...
  return ytdlpLimiter.run(async () => {
    ...
    try {
      ...
      args.push(url);
      const res = await execa(ytDlpPath, args, { cwd: workDir, timeout: 15_000, reject: false });
...
async function transcribeWithWhisper(rawUrl: string, providerId: string): Promise<CaptionsResult> {
  ...
  async getCaptions(rawUrl: string, lang = "en"): Promise<CaptionsResult> {
    ...
    logger.info({ videoId: rawUrl.slice(0, 60), lang, tag: "captions:request:start" }, "captions request start");
    ...
    if (providerId === "youtube" && videoId) {
      ...
      logger.info({ videoId, step: "native", durationMs: Date.now() - tNative, hit: !!native }, "captions step native done");
      if (native) {
        recordStatus({ endpoint: "captions", path: "native_captions", success: true });
        logger.info({ videoId, totalMs: Date.now() - tStart }, "captions request done via native");
        return native;
      }
      const tYt = Date.now();
      const viaYtDlp = await fetchNativeViaYtDlp(url, videoId);
      logger.info({ videoId, step: "yt-dlp-subs", durationMs: Date.now() - tYt, hit: !!viaYtDlp }, "captions step yt-dlp subs done");
      if (viaYtDlp) {
        recordStatus({ endpoint: "captions", path: "native_captions", success: true });
        logger.info({ videoId, totalMs: Date.now() - tStart }, "captions request done via yt-dlp");
        return viaYtDlp;
      }
      // Piped/Invidious fallback — no yt-dlp, no cookies, no datacenter IP block
      const tPiped = Date.now();
      const piped = await fetchPipedCaptions(videoId, lang);
      logger.info({ videoId, step: "piped", durationMs: Date.now() - tPiped, hit: !!piped }, "captions step piped done");
      if (piped) {
        recordStatus({ endpoint: "captions", path: "piped", success: true });
        logger.info({ videoId, totalMs: Date.now() - tStart, source: piped.source }, "captions request done via piped");
        return piped;
      }
    }
    ...
    // Keep total captions request <15s: if any path hangs, fail fast.
    const tWhisper = Date.now();
    try {
      const res = await transcribeWithWhisper(url, providerId);
      logger.info({ videoId, step: "whisper", durationMs: Date.now() - tWhisper, hit: true }, "captions step whisper done");
      recordStatus({ endpoint: "captions", path: "whisper", success: true });
      logger.info({ videoId, totalMs: Date.now() - tStart }, "captions request done via whisper");
      return res;
    } catch (err) {
      logger.info({ videoId, step: "whisper", durationMs: Date.now() - tWhisper, hit: false, code: err instanceof AppError ? err.code : "UNKNOWN" }, "captions step whisper done");
      recordStatus({ endpoint: "captions", path: "whisper", success: false });
      logger.info({ videoId, totalMs: Date.now() - tStart, success: false }, "captions request failed total");
