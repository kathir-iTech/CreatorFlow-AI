...
async function fetchPipedCaptions(videoId: string, lang = "en"): Promise<CaptionsResult | null> {
  const tryOnePiped = async (base: string): Promise<CaptionsResult | null> => {
    try {
      const listRes = await fetchWithTimeout(`${base}/captions/${videoId}`, 5000);
      if (!listRes.ok) return null;
      const caps = (await listRes.json()) as Array<{ languageCode?: string; language?: string; url?: string; isAutoGenerated?: boolean; autoGenerated?: boolean }>;
      if (!Array.isArray(caps) || caps.length === 0) return null;
      const norm = (s?: string) => (s ?? "").toLowerCase().split("-")[0];
      const preferred = caps.find((c) => norm(c.languageCode ?? c.language) === lang && !(c.isAutoGenerated ?? c.autoGenerated));
      const autoPref = caps.find((c) => norm(c.languageCode ?? c.language) === lang);
      const picked = preferred ?? autoPref ?? caps.find((c) => !(c.isAutoGenerated ?? c.autoGenerated)) ?? caps[0];
      if (!picked?.url) return null;
      const capUrl = picked.url.startsWith("http") ? picked.url : `${base}${picked.url}`;
      const r = await fetchWithTimeout(capUrl, 5000);
      if (!r.ok) return null;
      const text = await r.text();
      let segs: CaptionSegment[] = [];
      ...
      segs = sanitizeSegments(segs);
      if (segs.length === 0) return null;
      logger.info({ videoId, count: segs.length, via: `piped:${base}` }, "piped captions hit");
      return {
        videoId,
        providerId: "youtube",
        source: "native",
        language: picked.languageCode ?? picked.language ?? lang,
        isAuto: !!(picked.isAutoGenerated ?? picked.autoGenerated),
        captions: segs,
        srt: toSrt(segs),
        vtt: toVtt(segs),
      };
